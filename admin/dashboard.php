      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <?php
            ...
            $booking_statistics_sql = "SELECT (SELECT (CASE WHEN COUNT(tr.transaction_id) IS NULL THEN 0 ELSE COUNT(tr.transaction_id) END) FROM transaction AS tr WHERE CURRENT_TIMESTAMP < tr.booking_start_date) AS total_pending_booking, (SELECT (CASE WHEN COUNT(tr.transaction_id) IS NULL THEN 0 ELSE COUNT(tr.transaction_id) END) FROM transaction AS tr WHERE CURRENT_TIMESTAMP BETWEEN tr.booking_start_date AND tr.booking_end_date) AS total_active_booking, (SELECT (CASE WHEN COUNT(tr.transaction_id) IS NULL THEN 0 ELSE COUNT(tr.transaction_id) END) FROM transaction AS tr WHERE CURRENT_TIMESTAMP > tr.booking_end_date) AS total_expired_booking, (SELECT (CASE WHEN COUNT(tr.transaction_id) IS NULL THEN 0 ELSE COUNT(tr.transaction_id) END) FROM transaction AS tr WHERE terminated_date IS NOT NULL) AS total_terminated_booking";

            $payment_statistics_sql = "SELECT (SELECT (CASE WHEN SUM(inv.total_payment) IS NULL THEN 0 ELSE SUM(inv.total_payment) END) FROM invoice AS inv WHERE inv.payment_status <= 2) AS total_pending_payment, (SELECT (CASE WHEN SUM(inv.total_payment) IS NULL THEN 0 ELSE SUM(inv.total_payment) END) FROM invoice AS inv WHERE inv.payment_status = 3) AS total_approved_payment, (SELECT (CASE WHEN SUM(inv.total_payment) IS NULL THEN 0 ELSE SUM(inv.total_payment) END) FROM invoice AS inv WHERE inv.payment_status = 4) AS total_rejected_payment";
